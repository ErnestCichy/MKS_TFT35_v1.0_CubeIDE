...
//
#if (TOUCH_ROTATION == 0)
CalibData_t CalibrationData = {0.124041, -0.000172, -13.8562, -0.0009266, -0.084627, 329.23}; // default calibration data
#endif
#if (TOUCH_ROTATION == 1)
...
uint16_t calA[] = {20, 20};	// Calibration points
uint16_t calB[] = {240, 300};
uint16_t calC[] = {460, 30};
#endif
...
{
  GFX_DrawCircle(calX, calY, 6, ILI_WHITE);
  GFX_DrawLine(calX-4, calY, calX+4, calY, ILI_WHITE);
  GFX_DrawLine(calX, calY-4, calX, calY+4, ILI_WHITE);
}
...
{
	uint8_t calCount = 0;
	ILI_ClearDisplay(ILI_BLACK); // Clear screen for black
	...
	while(calCount < 4) // GEt all points and calculate
	{
		...
		case 0: // 1st point
			...
			break;
		case 1: // 2nd point
			GFX_DrawFillRectangle(calA[0]-6, calA[1]-6, 13, 13, ILI_BLACK);
			...
			break;
		case 2: // 3rd point
			GFX_DrawFillRectangle(calB[0]-6, calB[1]-6, 13, 13, ILI_BLACK);
		...
		case 3: // calculate and save calibration data,
			GFX_DrawFillRectangle(calC[0]-6, calC[1]-6, 13, 13, ILI_BLACK);
